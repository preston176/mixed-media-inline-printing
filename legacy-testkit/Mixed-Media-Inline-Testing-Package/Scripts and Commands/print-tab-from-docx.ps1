...
<#
  ...
  for DEVMODE's dmMediaType (a newer field than dmDefaultSource/tray). If Tray 1 is
  configured at the printer's own control panel to feed Tab Paper by default (per
  PLAYBOOK.md's setup step), tray selection alone should be enough; if precise media-type
...
if ($null -eq $tTab) { Fail "Tray1 not found in the bin list above. Check -Printer." 5 }
Write-Host ("Tray1 bin id = {0}" -f $tTab) -ForegroundColor Green
...
Write-Host ""
Write-Host ("This will PHYSICALLY PRINT tabs {0}..{1} via Word on '{2}' (Tray 1, {3} copy/copies each)." -f $TabNumber, $lastTab, $Printer, $Copies) -ForegroundColor Yellow
Write-Host "  Load the physical 5-cut set so tab $TabNumber's position is on TOP -- the tray feeds top-first, and this prints in that same order."
